...
function App() {
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-6">
            <h1 className="text-3xl font-bold text-blue-700 mb-4">Smart Validator via React</h1>
            ...
            {records.length > 0 && (
                <>
                    ...
                    <div className="flex gap-4 mb-4">
                        ...
                        <button onClick={() => runValidation(true)} className="bg-purple-500 text-white px-4 py-2 rounded hover:bg-purple-600">
            ...
            {loading && <p className="text-gray-700">Validating...</p>}

            {!loading && results.length > 0 && (
                <div className="mt-6">
                    <p className="text-gray-800 mb-2">API Used: {keySource}</p>
                    <p className="mb-4">
                        Summary:<br />✅ {summary.ok} OK<br />❌ {summary.error} Errors
                    </p>
                    <hr className="mb-4" />
                    {results.map((result, idx) => (
                        <div key={idx} className="mb-4 p-4 bg-white shadow rounded">
                            <h3 className="text-lg font-semibold">Item #{idx + 1}</h3>
                            <p>Status: <span className={result.status === 'OK' ? 'text-green-600' : 'text-red-600'}>{result.status}</span></p>
